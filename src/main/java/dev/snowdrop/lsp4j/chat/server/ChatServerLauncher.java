 ...
 * ------------------------------------------------------------------------------------------ */
package dev.snowdrop.lsp4j.chat.server;
...
import dev.snowdrop.lsp4j.chat.shared.ChatClient;
import dev.snowdrop.lsp4j.chat.shared.SocketLauncher;
		...
		try (ServerSocket serverSocket = new ServerSocket(port)) {
			...
			threadPool.submit(() -> {
				while (true) {
                     ...
                     */
					launcher.startListening().get(); // TODO: Check what thenRun is about and should be replaced !!
				}
			});
